...
import { MailboxAccountsPanel } from './MailboxAccountsPanel';
...
export const CorrespondenceDashboard: React.FC = () => {
  ...
  const canManageMailboxes = userSession?.rol === 'Administrador' || userSession?.rol === 'Propietario';
        ...
        <header className="theme-surface rounded-3xl border p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              ...
              {selected.destinatarios && <p className="theme-muted mt-1 text-[10px]">Destinatarios originales: {selected.destinatarios}</p>}
            </div>
    ...
    <div className="space-y-6">
      ...
      <section className="theme-surface rounded-3xl border p-5">
        ...
        <form onSubmit={event => { event.preventDefault(); applyFilter('busqueda', searchDraft); }} className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <input value={filters.cuenta_origen || ''} onChange={event => applyFilter('cuenta_origen', event.target.value)} className="theme-input rounded-xl border px-3 py-3 text-xs" placeholder="Cuenta de Gmail receptora" />
      ...
      <section className="theme-surface rounded-3xl border p-5">
        <MailboxAccountsPanel canManage={canManageMailboxes} />
      </section>
    </div>
  );
};
